/* Quran Reels Studio 5 — deterministic fixed-frame browser editor */
(() => {
'use strict';

const PRO_VERSION='5.0.0';
const STORAGE_KEY='quran-reels-studio-v5';
...
Object.assign(S,{
  ...
  exportType:'mp4',exportQuality:'high',exportBitrate:8,previewQuality:'balanced',
...
draw=function(){
  ...
  const fmt=FMTS[S.fmt]||FMTS['9:16'],w=fmt.w,h=fmt.h,sx=CV.width/w,sy=CV.height/h;
  CT.setTransform(1,0,0,1,0,0);CT.clearRect(0,0,CV.width,CV.height);CT.setTransform(sx,0,0,sy,0,0);
  drawBg(w,h);CT.fillStyle=`rgba(0,0,0,${S.veil})`;CT.fillRect(0,0,w,h);drawGrain(w,h);
...
const AAC_ENCODER_URL='https://cdn.jsdelivr.net/npm/@mediabunny/aac-encoder@1.52.3/+esm';
let mediabunnyPromise=null;
function loadMediabunny(){return mediabunnyPromise||(mediabunnyPromise=import(MEDIABUNNY_URL));}
function ensureExportModal(){
  let m=$('proExportModal');if(m)return m;m=document.createElement('div');m.id='proExportModal';m.className='pro-modal';
  m.innerHTML=`<div class="pro-dialog"><div class="pro-dialog-h"><h3>Export Quran Reel</h3><button class="pro-btn" onclick="QRSPro.closeExport()">✕</button></div><div class="pro-dialog-b"><div class="pro-export-grid"><div id="expMp4" class="pro-export-card" onclick="QRSPro.chooseExport('mp4')"><strong>MP4 · H.264/AAC</strong><p>Deterministic fixed-FPS render. AAC is polyfilled when Chromium has no native encoder.</p></div><div id="expWebm" class="pro-export-card" onclick="QRSPro.chooseExport('webm')"><strong>WebM · VP9/Opus</strong><p>Fixed-FPS browser render with guaranteed Opus audio when supported.</p></div></div><div class="cg" style="margin-top:10px"><label>Quality</label><div class="pro-segmented">${['draft','high','ultra'].map(q=>`<button id="q_${q}" onclick="QRSPro.chooseQuality('${q}')">${q}</button>`).join('')}</div><p class="pro-help">Draft: 540p/24fps · High: 720p/30fps · Ultra: 1080p/30fps. 30fps is deliberate: smoother and more reliable than the old 60fps real-time capture.</p></div><div class="cg"><label>Editor preview performance</label><div class="pro-segmented">${['performance','balanced','quality'].map(q=>`<button id="pv_${q}" onclick="QRSPro.choosePreviewQuality('${q}')">${q}</button>`).join('')}</div></div><div class="pro-export-note" id="proExportSupport">Checking encoders…</div></div><div class="pro-dialog-f"><button class="pro-btn" onclick="QRSPro.closeExport()">Cancel</button><button class="pro-btn green" onclick="QRSPro.startExport()">Render & download</button></div></div>`;
  document.body.appendChild(m);return m;
}
window.doExport=async()=>{
  const m=ensureExportModal();m.classList.add('on');chooseExport(S.exportType);chooseQuality(S.exportQuality);choosePreviewQuality(S.previewQuality||'balanced',false);
  const support=$('proExportSupport');
  try{
    const mb=await loadMediabunny();
    const avc=await mb.canEncodeVideo('avc'),vp9=await mb.canEncodeVideo('vp9'),opus=await mb.canEncodeAudio('opus'),aac=await mb.canEncodeAudio('aac');
    support.textContent=`Detected: H.264 ${avc?'✓':'—'} · native AAC ${aac?'✓':'polyfill needed'} · VP9 ${vp9?'✓':'—'} · Opus ${opus?'✓':'—'}. Export is frame-by-frame and does not depend on tab frame rate.`;
  }catch(e){support.textContent='The media engine will load when export starts. An internet connection is needed the first time unless the library is cached.';}
};
...
function choosePreviewQuality(q,redraw=true){S.previewQuality=q;['performance','balanced','quality'].forEach(x=>$(`pv_${x}`)?.classList.toggle('on',x===q));saveSettings();if(redraw){resizeCV(false);draw();}}
function qualityConfig(){
  const fmt=FMTS[S.fmt]||FMTS['9:16'];
  const scale=S.exportQuality==='draft'?.5:S.exportQuality==='ultra'?1:2/3;
  return{fps:S.exportQuality==='draft'?24:30,bps:S.exportQuality==='draft'?2_800_000:S.exportQuality==='ultra'?10_000_000:6_000_000,width:Math.max(2,Math.round(fmt.w*scale/2)*2),height:Math.max(2,Math.round(fmt.h*scale/2)*2)};
}
function createImpulse(ctx,seconds=S.roomSize||1.1,decay=S.roomDecay||3){const rate=ctx.sampleRate,len=Math.max(1,Math.floor(rate*seconds)),buf=ctx.createBuffer(2,len,rate);let seed=2166136261;for(let c=0;c<2;c++){const d=buf.getChannelData(c);for(let i=0;i<len;i++){seed^=i+c*131;seed=Math.imul(seed,16777619);const noise=((seed>>>0)/4294967295)*2-1;d[i]=noise*Math.pow(1-i/len,decay);}}return buf;}
...
function makeTimeline(data){
  const idx=selectedIndices(),intro=S.introEnabled?S.introDuration:0,outro=S.outroEnabled?S.outroDuration:0,starts=[],ends=[];let cursor=intro;
  if(data.custom){for(const d of data.durations){starts.push(cursor);cursor+=d;ends.push(cursor);}}
  else{for(let i=0;i<idx.length;i++){starts.push(cursor);cursor+=data.buffers[i]?.duration||2.5;ends.push(cursor);cursor+=S.verseHold;}}
  return{idx,intro,outro,starts,ends,cursor,total:cursor+outro+.25};
}
async function renderMasterAudio(data,timeline){
  const rate=48000,frames=Math.max(1,Math.ceil(timeline.total*rate)),offline=new OfflineAudioContext(2,frames,rate),master=createMasterChain(offline,offline.destination);
  if(data.custom){const src=offline.createBufferSource(),g=offline.createGain();src.buffer=data.buffers[0];g.gain.value=peakGain(src.buffer);src.connect(g).connect(master);src.start(timeline.intro);}
  else data.buffers.forEach((buf,i)=>{if(!buf)return;const src=offline.createBufferSource(),g=offline.createGain();src.buffer=buf;g.gain.value=peakGain(buf);src.connect(g).connect(master);src.start(timeline.starts[i]);});
  updateExport(.17,'Rendering audio master…');return offline.startRendering();
}
function applyTimelineState(t,tl){
  if(t<tl.intro){S._phase='intro';S._verseProgress=clamp(t/Math.max(.01,tl.intro),0,1);return;}
  if(t>=tl.cursor){S._phase='outro';S._verseProgress=clamp((t-tl.cursor)/Math.max(.01,tl.outro),0,1);return;}
  S._phase='verse';let i=tl.starts.findIndex((st,k)=>t>=st&&t<(tl.ends[k]??st));
  if(i<0){i=Math.max(0,tl.starts.findLastIndex(st=>t>=st));S._verseProgress=1;}else S._verseProgress=clamp((t-tl.starts[i])/Math.max(.001,tl.ends[i]-tl.starts[i]),0,1);
  S.vIdx=clamp(i,0,tl.idx.length-1);
}
let deterministicDrawBgInstalled=false;
function installDeterministicBackgroundHook(){
  if(deterministicDrawBgInstalled)return;deterministicDrawBgInstalled=true;const previous=window.drawBg;
  window.drawBg=function(w,h){
    if(S._deterministicBgFrame){CT.save();CT.filter=S.bgBlur?`blur(${S.bgBlur}px)`:'none';CT.drawImage(S._deterministicBgFrame,0,0,w,h);CT.filter='none';if(S.vignette>0){const g=CT.createRadialGradient(w/2,h/2,w*.3,w/2,h/2,w*.72);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,`rgba(0,0,0,${S.vignette})`);CT.fillStyle=g;CT.fillRect(0,0,w,h);}if(S.colorOv){CT.globalAlpha=S.colorOvOp;CT.fillStyle=S.colorOv;CT.fillRect(0,0,w,h);}CT.restore();return;}
    return previous(w,h);
  };
}
async function getBackgroundBlob(){
  if(!S._bgV)return null;const src=S._bgV.currentSrc||S._bgV.src;if(!src)return null;
  const r=await fetch(src);if(!r.ok)throw new Error(`Background download failed (${r.status})`);return r.blob();
}
async function createBackgroundPlan(mb,frameCount,fps){
  if(!S._bgV)return{type:'native',dispose(){}};
  try{
    updateExport(.19,'Preparing background video…');const blob=await getBackgroundBlob();const input=new mb.Input({formats:mb.ALL_FORMATS,source:new mb.BlobSource(blob)});const track=await input.getPrimaryVideoTrack();if(!track)throw new Error('No video track in background');
    const duration=await input.computeDuration([track]);if(!Number.isFinite(duration)||duration<=0)throw new Error('Unknown background duration');
    const sink=new mb.CanvasSink(track,{width:CV.width,height:CV.height,fit:'cover',poolSize:1});const speed=Math.max(.05,S.bgVidSpd||1);
    async function* frames(){
      let start=0;
      while(start<frameCount){
        const cycle=Math.floor((start/fps*speed)/duration),segment=[];let i=start;
        for(;i<frameCount;i++){const raw=i/fps*speed;if(Math.floor(raw/duration)!==cycle)break;segment.push(raw-cycle*duration);}
        const iterator=sink.canvasesAtTimestamps(segment);let j=0;
        for await(const wrapped of iterator){yield wrapped?.canvas||null;j++;}
        while(j<segment.length){yield null;j++;}
        start=i;
      }
    }
    return{type:'video',frames:frames(),dispose(){try{input.dispose();}catch(_){}}};
  }catch(error){console.warn('Background video decode fallback:',error);toast('Video background could not be decoded for export; using a smooth generated background instead. Import the video locally for exact export.','bad');return{type:'fallback',dispose(){}};}
}
async function ensureAac(mb){
  if(await mb.canEncodeAudio('aac'))return;
  updateExport(.2,'Loading reliable AAC encoder…');
  try{const ext=await import(AAC_ENCODER_URL);ext.registerAacEncoder();}catch(first){const ext=await import('https://cdn.jsdelivr.net/npm/@mediabunny/aac-encoder/+esm');ext.registerAacEncoder();}
  if(!(await mb.canEncodeAudio('aac')))throw new Error('AAC encoder unavailable. Choose WebM to keep sound.');
}
async function validateOutput(mb,blob,expectedType,expectedFps){
  const input=new mb.Input({formats:mb.ALL_FORMATS,source:new mb.BlobSource(blob)});
  try{
    const [video,audio,duration]=await Promise.all([input.getPrimaryVideoTrack(),input.getPrimaryAudioTrack(),input.computeDuration()]);
    if(!video)throw new Error('Export validation failed: no video track');
    if(!audio)throw new Error('Export validation failed: no audio track');
    if(!Number.isFinite(duration)||duration<.5)throw new Error('Export validation failed: invalid duration');
    const [audioDuration,stats]=await Promise.all([audio.computeDuration(),video.computePacketStats(180)]);
    if(!Number.isFinite(audioDuration)||audioDuration<duration-1)throw new Error('Export validation failed: audio track is shorter than the video');
    const measuredFps=stats?.averagePacketRate||0;
    if(expectedFps&&measuredFps<expectedFps*.9)throw new Error(`Export validation failed: only ${measuredFps.toFixed(1)}fps was written`);
    return{duration,audioDuration,measuredFps,type:expectedType};
  }finally{try{input.dispose();}catch(_){}}
}
async function deterministicExport(){
  const idx=selectedIndices();if(!idx.length)throw new Error('Select at least one ayah');const q=qualityConfig(),decodeCtx=new AudioContext();let output=null,bgPlan=null;
  const old={w:CV.width,h:CV.height,phase:S._phase,vIdx:S.vIdx,progress:S._verseProgress};
  try{
    const data=await loadBuffers(decodeCtx,(a,b)=>updateExport(.02+.12*a/b,`Loading recitation ${a}/${b}`));if(!data.custom&&!data.buffers.some(Boolean))throw new Error('Recitation audio could not be loaded');S.durations=data.durations;renderTimeline();
    const timeline=makeTimeline(data),audioBuffer=await renderMasterAudio(data,timeline),mb=await loadMediabunny();
    if(S.exportType==='mp4')await ensureAac(mb);
    const videoCodec=S.exportType==='mp4'?'avc':(await mb.canEncodeVideo('vp9')?'vp9':'vp8'),audioCodec=S.exportType==='mp4'?'aac':'opus';
    if(!(await mb.canEncodeVideo(videoCodec)))throw new Error(`${videoCodec.toUpperCase()} video encoder unavailable in this browser`);if(!(await mb.canEncodeAudio(audioCodec)))throw new Error(`${audioCodec.toUpperCase()} audio encoder unavailable in this browser`);
    S._fullResolution=true;CV.width=q.width;CV.height=q.height;CV.dataset.logicalWidth=(FMTS[S.fmt]||FMTS['9:16']).w;CV.dataset.logicalHeight=(FMTS[S.fmt]||FMTS['9:16']).h;installDeterministicBackgroundHook();
    const frameCount=Math.ceil(timeline.total*q.fps),format=S.exportType==='mp4'?new mb.Mp4OutputFormat({fastStart:'in-memory'}):new mb.WebMOutputFormat(),target=new mb.BufferTarget();
    output=new mb.Output({format,target});const videoSource=new mb.CanvasSource(CV,{codec:videoCodec,bitrate:q.bps,hardwareAcceleration:'prefer-hardware'}),audioSource=new mb.AudioBufferSource({codec:audioCodec,bitrate:192000});
    output.addVideoTrack(videoSource,{frameRate:q.fps});output.addAudioTrack(audioSource,{languageCode:'ara',name:'Quran recitation'});output.setMetadataTags({title:`${S.surah?.[2]||'Quran'} recitation`,artist:'Quran Reels Studio'});await output.start();
    bgPlan=await createBackgroundPlan(mb,frameCount,q.fps);if(bgPlan.type==='fallback'){S._deterministicBgFrame=null;const oldType=S.bgT;S._exportFallbackBgType=oldType;S.bgT='motion';S.motionBg=S.motionBg||'nur-particles';}
    const audioJob=audioSource.add(audioBuffer).then(()=>audioSource.close());let bgIterator=bgPlan.type==='video'?bgPlan.frames[Symbol.asyncIterator]():null;
    for(let frame=0;frame<frameCount;frame++){
      if(S._cancel){await output.cancel();throw new Error('cancelled');}
      const t=frame/q.fps;S._renderTime=t;applyTimelineState(t,timeline);
      if(bgIterator){
        const next=await bgIterator.next();
        if(next.value)S._deterministicBgFrame=next.value;
        else if(frame===0){bgIterator=null;S._exportFallbackBgType=S.bgT;S.bgT='motion';S.motionBg=S.motionBg||'nur-particles';toast('The selected background had no decodable first frame; a smooth generated background was used.','bad');}
      }
      draw();await videoSource.add(t,1/q.fps,{keyFrame:frame%(q.fps*2)===0});
      if(frame%Math.max(1,Math.floor(q.fps/3))===0){updateExport(.22+.7*(frame/frameCount),`Encoding frame ${frame+1}/${frameCount} · ${formatTime(t)}`);await new Promise(r=>setTimeout(r,0));}
    }
    videoSource.close();await audioJob;updateExport(.94,'Finalizing container…');await output.finalize();const mime=S.exportType==='mp4'?'video/mp4':'video/webm',blob=new Blob([target.buffer],{type:mime});updateExport(.98,'Validating audio and video tracks…');const verified=await validateOutput(mb,blob,S.exportType,q.fps);S._lastExportValidation=verified;return blob;
  }finally{
    try{await decodeCtx.close();}catch(_){}try{bgPlan?.dispose();}catch(_){}S._deterministicBgFrame=null;S._renderTime=NaN;if(S._exportFallbackBgType){S.bgT=S._exportFallbackBgType;delete S._exportFallbackBgType;}S._fullResolution=false;resizeCV(false);S._phase=old.phase||'verse';S.vIdx=old.vIdx||0;S._verseProgress=old.progress||0;draw();
  }
}
...
async function startExport(){
  closeExport();if(S.exporting)return;$('exo').classList.add('on');$('expT').textContent='Rendering fixed-frame video';$('cnlBtn').style.display='inline-block';$('ppBtn').disabled=true;$('peBtn').disabled=true;S.exporting=true;S._cancel=false;
  try{const blob=await deterministicExport();if(S._cancel)throw new Error('cancelled');const ext=S.exportType==='mp4'?'mp4':'webm';downloadBlob(blob,projectName(ext));updateExport(1,'Complete · audio and video verified');$('expT').textContent='Export complete';const fps=S._lastExportValidation?.measuredFps?.toFixed(1)||'fixed';toast(`${ext.toUpperCase()} verified: audio + ${fps}fps video`);await new Promise(r=>setTimeout(r,500));}
  catch(e){console.error(e);$('expT').textContent=e.message==='cancelled'?'Export cancelled':'Export failed';updateExport(0,e.message==='cancelled'?'Cancelled':e.message);toast(e.message==='cancelled'?'Export cancelled':e.message,'bad');}
  finally{S.exporting=false;S._cancel=false;S._phase='verse';$('exo').classList.remove('on');$('ppBtn').disabled=false;$('peBtn').disabled=false;$('cnlBtn').style.display='none';draw();}
}
...
// ---------- Public API and boot ----------
window.QRSPro={selectRange,changeTranslation,jumpVerse,analyzeAudio,toggleSection,applyTemplate,setValue,toggleTransliteration,filterReciters,loadCustomAudio,clearCustomAudio,toggleMicRecord,savePexelsKey,closeExport,chooseExport,chooseQuality,choosePreviewQuality,startExport,exportProject,importProject,resetProject};
